...
#   inst_opcode, inst_rs, inst_rt, inst_rd, inst_shamt, inst_funct, inst_imm, inst_addr
#
...
#   $t0-$t7, $a0-$a3, $v0-$v1 : usados livremente dentro de cada bloco
#   $ra : salvo/restaurado nos procedimentos que chamam outros via jal
#   $t8 : argumento de entrada para 'transfere_sim_para_host' (endereço simulado)
#   $t9 : usado para salvar endereço host antes de novo jal em exec_sw/exec_sb
##############################################################

################################
# Diretrizes .eqv para o código
###############################

# Endereços base dos segmentos de memória simulados
.eqv  TEXT_BASE,   0x00400000   # Base do segmento de texto simulado
.eqv  DATA_BASE,   0x10010000   # Base do segmento de dados simulado
.eqv  STACK_TOP,   0x7FFFEFFC   # Topo inicial da pilha simulada

# Tamanho de cada segmento de memória (em bytes)
.eqv  MEM_SIZE,    4096

# Número de registradores simulados
.eqv  NUM_REGS,    32

# Índices de registradores especiais (usados por get_reg/set_reg)
.eqv  IDX_V0,      2            # $v0 (código de syscall/retorno)
.eqv  IDX_A0,      4            # $a0 (primeiro argumento)
.eqv  IDX_A1,      5            # $a1 (segundo argumento)
...
main:
    ...
    la $s6, reg # $s6 = endereço host de reg[0]
    la $s7, PC # $s7 = endereço host de PC
...
limpar_regs_loop:
    ...
    addiu $t0, $t0, 4
    addiu $t1, $t1, -1
    bgtz $t1, limpar_regs_loop

    # Inicializar reg[29] (STACK_TOP = 0x7FFFEFFC)
    # Poderia ser feito com 'lui' + 'ori', mas utilizando 'li' permite eu utilizar o '.eqv' que define o STACK_TOP
    li $t0, STACK_TOP
    sw $t0, 116($s6)   # 29 * 4 = 116 (reg[29] = STACK_TOP)
    ...
    # Ler arquivo '.bin' para 'mem_text'
    la $a0, arquivo_bin
    la  $a1, mem_text
    li $a2, MEM_SIZE
    jal carregar_arquivo

    # Ler arquivo '.dat' para 'mem_data'
    la $a0, arquivo_dat
    la $a1, mem_data
    li $a2, MEM_SIZE
    jal carregar_arquivo
    ...
    #########################################################
    # 3. Loop Principal de Execução: Fetch - Decode - Execute
    #########################################################
exec_loop:

    # Busca (FETCH): Mem[PC] -> IR
exec_fetch:
    lw $t0, 0($s7)             # $t0 = PC simulado
    li $t1, TEXT_BASE          # TEXT_BASE = 0x00400000
    ...
    la $t4, IR                 # Carrega endereço de IR
    sw $t3, 0($t4)             # Salva a instrução de $t3 para o endereço do IR

    # Decodificação (DECODE): extrair campos de IR e incrementar PC
exec_decode:
    # PC = PC + 4
    addiu $t0, $t0, 4
    sw $t0, 0($s7)
    ...
    # opcode = IR[31-26]
    srl $t4, $t3, 26            # Desloca para direita, para sobrar só OP
    andi $t4, $t4, 0x3F         # 0x3F = 0011 1111 (máscara pra manter os 6 primeiros)
    la $t0, inst_opcode
    sw $t4, 0($t0)

    # rs = IR[25-21]
    srl $t4, $t3, 21            # Desloca para direita, sobrando os bits mais significativos
    andi $t4, $t4, 0x1F         # 0x1F = 0001 1111 (faz sobrar só 5 bits)
    la $t0, inst_rs
    sw $t4, 0($t0)

    # rt = IR[20-16]
    srl $t4, $t3, 16            # Mesma lógica dos 2 primeiros campos
    andi $t4, $t4, 0x1F
    la $t0, inst_rt
    sw $t4, 0($t0)

    # rd = IR[15-11]
    srl $t4, $t3, 11           # Mesma lógica dos 2 primeiros campos
    andi $t4, $t4, 0x1F
    la $t0, inst_rd
    sw $t4, 0($t0)

    # shamt = IR[10-6]
    srl $t4, $t3, 6            # Mesma lógica dos 2 primeiros campos
    andi $t4, $t4, 0x1F
    la $t0, inst_shamt
    sw  $t4, 0($t0)

    # funct = IR[5-0]
    andi $t4, $t3, 0x3F         # Só precisa fazer o 'andi' com 5 bits menos significativos
    la $t0, inst_funct
    sw $t4, 0($t0)
    ...
    sll $t4, $t3, 16 
    sra $t4, $t4, 16            # Precisa fazer assim para a extensão de sinal dos 16 bits mais significativos
    la $t0, inst_imm
    sw $t4, 0($t0)

    # address = IR[25-0]  (J-type, 26 bits sem sinal)
    li $t0, 0x03FFFFFF         # Não da pra fazer só com 'andi' pois é maior que 16 bits (não cabe no imm)
    and $t4, $t3, $t0
    la $t0, inst_addr
    sw $t4, 0($t0)

    # Execução (EXECUTE): despachar pelo opcode
exec_execute:
    la $t0, inst_opcode
    ...
    la $a0, msg_opcode_desconhecido
    li $v0, 4
    syscall
    j exec_loop # Volta para o inicio do loop, tentando encontrar mais erros
##############################################################################
...
exec_rtype:
    ...
    la $a0, msg_funct_desconhecido
    li $v0, 4
    syscall
    j exec_loop # Tenta encontrar mais erros
    ...
    # ----- jr rs : PC = reg[rs] -----
exec_jr:
    la $t0, inst_rs        # Carrega o endereço do 'rs' 
    lw  $a0, 0($t0)         # Pega o valor do 'rs'
    jal get_reg             # $v0 = reg[rs] (pega o valor do registrador no índice 'rs')
    sw $v0, 0($s7)         # PC = reg[rs]
    j exec_loop

    # ----- syscall : verificar reg[2] (v0) e executar o serviço -----
exec_syscall:
    li $a0, IDX_V0
    jal get_reg             # $v0 = reg[2] (código de serviço)
    move $t0, $v0           # $t0 toma o valor de $v0 (pseudoinstrucao)
    ...
    j exec_loop           # Ignorar serviço desconhecido mas continuar

    # ---- Serviço 1: print_int
svc_print_int:
    li $a0, IDX_A0
    jal get_reg
    move $a0, $v0          # Printa o inteiro em $a0
    li $v0, 1              # Serviço 1
    syscall 
    j exec_loop
...
svc_print_str:
    ...
    move $t8, $v0
    jal transfere_sim_para_host         # $v0 = endereço host
    move $a0, $v0                       # Printa o char com endereço em a0
    li $v0, 4                           # Serviço 4
    syscall
    j exec_loop
...
svc_read_int:
    ...
    syscall
    li $a0, IDX_V0
    move $a1, $v0           # $v0 tem o int lido, vai pra $a1 pra ser passado por função
...
svc_read_str:
    ...
    jal get_reg
    move $t8, $v0
    jal transfere_sim_para_host
    move $t9, $v0                # $t9 = endereço host do buffer
    li $a0, IDX_A1
    jal get_reg                  # $v0 = tamanho (reg[5])
    move $a1, $v0                # $a1 tem o numero de caracteres da string
    move $a0, $t9                # $a0 precisa ter o endereço do input buffer
    li $v0, 8                    # Serviço 8
    syscall
    j exec_loop
...
svc_print_char:
    li $a0, IDX_A0
    jal get_reg
    move $a0, $v0        # Caractere pra printar
    li $v0, 11           # Serviço 11
    syscall
    j exec_loop
...
svc_exit2:
    li $a0, IDX_A0
    jal get_reg
    move $a0, $v0        # Termina com o valor de $a0
    li $v0, 17           # Serviço 17
    syscall
...
exec_addu:
    ...
    lw $a0, 0($t0)
    jal get_reg            
    move $t2, $v0               # $t2 = reg[rs]
    addu $t1, $t1, $t2
    la $t0, inst_rd
    lw $a0, 0($t0)
    move $a1, $t1
    jal set_reg                 # $a0 = conteúdo do inst_rd | $a1 = reg[rs] + reg[rs]
    j exec_loop

    # ----- subu rd, rs, rt : reg[rd] = reg[rs] - reg[rt] -----
exec_subu:
    # Parecido com o 'addu', mas agora com subtração
    la $t0, inst_rs
    lw $a0, 0($t0)
    jal get_reg
    move $t1, $v0
    ...
    # Nota: PC já foi incrementado no Decode, usamos esse valor para extrair os 4 bits superiores
exec_j:
    la $t0, inst_addr
...
#   $a0 : índice do registrador (0-31)
#   $a1 : valor a gravar
# Nota: reg[0] é imutável (escrita ignorada), sempre zero
# Registradores preservados: $s6, $s7, $t5-$t9, $a0-$a3
#######################################################
set_reg:
...
# Retorno: 
#   $v0 : endereço host correspondente
#
# Mapeamento de segmentos:
#   Texto : [TEXT_BASE, TEXT_BASE + MEM_SIZE)
#   Dados : [DATA_BASE, DATA_BASE + MEM_SIZE)
...
transfere_sim_para_host:
    ...
    # ---- Segmento de texto ----
transfere_sim_para_host_texto:
    li $t0, TEXT_BASE                            # Carrega 0x00400000
    subu $t1, $t8, $t0                           # offset = addr - TEXT_BASE
    bltz $t1, transfere_sim_para_host_data       # offset < 0 , não é texto
    li $t2, MEM_SIZE
